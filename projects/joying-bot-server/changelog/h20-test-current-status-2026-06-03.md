...
## 2026-06-03 15:08 h20 测试服新增第四组 Docker 模型服务

本次在 h20 当前部署目录追加并启动第四组 Docker 模型服务，用于把测试服视频生成并发从 3 路扩到 4 路：

- compose 文件：`/data/project/test_ai_botserver/deploy/docker/docker-compose.h20.yml`
- 当前部署目录：`/data/project/test_ai_botserver.20260603145500`
- 远程备份：`deploy/docker/docker-compose.h20.yml.bak.20260603150801`
- 新增容器：
  - `voxcpm-api-h20-test-4`，端口 `8126`，GPU0，健康检查 ok
  - `latentsync-api-h20-test-4`，端口 `8127`，GPU7，健康检查 ok
- 测试库新增资源池：`zhugedata_test.t_comfyui_config.id=11`
  - `config_value_audio=http://127.0.0.1:8126`
  - `config_value=http://127.0.0.1:8127`
  - `is_active=1`
- 当前 active Docker 模型池：`id=1`、`id=2`、`id=10`、`id=11`，对应 `8120/8121`、`8122/8123`、`8124/8125`、`8126/8127`。
- 验证结果：`8120-8127 /health` 均返回 `{"status":"ok"}`；新增第 4 组 Docker 状态均为 healthy。

注意：第 4 组 VoxCPM 绑定 GPU0，GPU0 当前还有旧裸机/回滚相关进程占显存；LatentSync 绑定 GPU7。现场 compose 已补齐到 4 组，但仍需后续固化回 Git 或生产运维目录，避免自动部署覆盖。
