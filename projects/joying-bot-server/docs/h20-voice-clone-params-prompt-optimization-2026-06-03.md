...
## 2026-06-04 长文案音色情绪变淡排查与待办

### 问题现象

产品反馈：音色克隆情绪提示词在短文案上体现比较明显，但文案一长，情绪影响变弱，整体听起来偏平淡。

本次已在 h20 测试服做只读排查，确认不是“代码未更新”或“VoxCPM Docker 未重启”的问题。此前已将 `test` 合并到最新提交 `ace9a2bf fix: update VoxCPM emotion prompts`，并重启完整视频池与试听池 VoxCPM 容器：
...
试听池最近日志中，非默认情绪确实进入 `mode=controllable`，并且新提示词已经被拼入文本开头：

```text
mode=controllable text=(温柔亲切，自然随和，语速平缓，带着真诚的微笑感，自然语速)... emotion_id=2
mode=controllable text=(热情洋溢，充满活力，语速稍快，声音明亮有感染力，自然语速)... emotion_id=3
mode=controllable text=(激情澎湃，铿锵有力，节奏感强烈，富有号召力和感染力，带着逐渐密集的节奏感，自然语速)... emotion_id=4
mode=controllable text=(严肃沉稳，语速适中偏慢，字字清晰，不带多余情绪，客观权威，自然语速)... emotion_id=5
mode=controllable text=(欢快喜悦，轻松愉快，语速轻快，声音明亮，带着抑制不住的开心，自然语速)... emotion_id=6
mode=controllable text=(低沉哀伤，语速缓慢，声音略带沙哑，语调下沉，情绪深沉克制，自然语速)... emotion_id=7
```

完整视频池最近日志看到的请求则基本是：

```text
mode=hifi emotion_id=1
```

DB 最近 1 天 `t_video_generate_task` 的分布也显示：
...
- 如果产品测的是试听接口：非默认情绪已经进入 controllable 模式，但长文案会被稀释。
- 如果产品测的是完整视频：最近完整视频任务并没有拿到非默认情绪，全是 `voice_emotion=1`，所以不会体现情绪提示词。

### 根因判断

#### 1. 情绪提示词只在整段文本开头出现一次

当前 `router/service/video_server/voxcpm_api.py` 中：

```python
return f"({emotion_prompt}，{speed_prompt}){body_text}"
```
...
```text
(激情澎湃，铿锵有力，节奏感强烈，...，自然语速)正文正文正文正文正文正文正文正文正文...
```

短文案时，情绪提示词离全文很近，模型容易跟随；文案一长，提示词只在最开头，后续几百字没有再次强化，模型更容易回到平稳朗读。

#### 2. 默认语速会追加 `自然语速`，和情绪模板里的语速描述冲突

产品新模板本身已经包含语速/节奏控制，例如：
...
```text
(热情洋溢，充满活力，语速稍快，声音明亮有感染力，自然语速)
(低沉哀伤，语速缓慢，声音略带沙哑，语调下沉，情绪深沉克制，自然语速)
```

这个“自然语速”会削弱热情/激昂/悲伤/愤怒等情绪表达，长文案时稀释更明显。

#### 3. 完整视频链路近期样本未验证到非默认情绪

最近 1 天完整视频任务表里全部是 `voice_emotion=1`、`voice_speed=1.0`。如果产品反馈来自完整视频生成结果，需要进一步确认 CRM `generateTaskList` 返回体、Bot 同步任务、任务入库是否真的带了 `voice_emotion`。

### 待办：解决方案

#### P0：先修默认语速冲突

目标：非默认情绪下，如果用户没有主动改语速，即 `voice_speed=1.0`，不要再拼 `自然语速`。

建议规则：

```python
if emotion_id != 1 and speed == 1.0:
    return f"({emotion_prompt}){body_text}"
return f"({emotion_prompt}，{speed_prompt}){body_text}"
```
...
```text
当前：
(热情洋溢，充满活力，语速稍快，声音明亮有感染力，自然语速)正文

改后：
(热情洋溢，充满活力，语速稍快，声音明亮有感染力)正文
```

当用户显式选择 `0.75 / 1.25 / 1.5 / 2.0 / 3.0` 时，再追加 speed prompt，表示用户主动覆盖或强化语速。

需要补单测：

- 非默认情绪 + `voice_speed=1.0` 不拼 `自然语速`。
- 默认情绪 + `voice_speed=1.0` 仍保持默认行为。
- 非默认情绪 + `voice_speed=1.25/1.5/...` 仍拼对应语速提示。

#### P0：确认完整视频链路是否拿到非默认情绪
...
- `t_video_generate_task.voice_emotion` 是否入库为 4。
- scheduler 调用 VoxCPM 日志是否出现 `mode=controllable emotion_id=4`。
- 如果仍是 `mode=hifi emotion_id=1`，继续追 CRM `generateTaskList` 返回和 Bot 同步字段。

#### P1：长文案分段生成，分段重复情绪控制

根治长文案情绪稀释，需要将长文本切成多个片段，每段单独加同一情绪提示词并调用 VoxCPM，然后拼接音频。

建议规则：

```text
文本长度 <= 120 字：保持单次生成
文本长度 > 120 字：按句号/问号/感叹号/逗号附近切分，每段 80-120 字
每段都用同一个 style prompt
每段单独 VoxCPM generate
最后拼接 numpy audio，可插入 80-150ms 短静音
```

不要在同一次长文本里反复插入括号提示词，因为 VoxCPM 有概率把括号里的内容当正文读出来。更稳的是“多次生成 + 音频拼接”。

需要注意：

- 试听接口长文案分段后会占用模型更久，需要继续走试听池并发保护。
- 完整视频链路分段后生成时长可能更长，需要观察超时。
- 分段之间音色一致性通常可以接受，因为参考音频相同，但需要实测。
- 音量和 speed 后处理建议放在每段生成后或拼接后统一做，需比较效果。

#### P1：增加诊断日志

当前日志只打印 `styled_text[:50]`，排查长文案不够。
...
避免打印完整长文案，防止日志过大和敏感内容泄露。

#### P2：试听/完整视频各做一组主观对比样本

需要固定同一个参考音频，分别测：

```text
短文案 30-50 字：亲切 / 热情 / 激昂 / 严肃 / 喜悦 / 悲伤 / 愤怒
长文案 300-500 字：同上
```
...
输出 CDN wav，并记录产品主观评价。

### 推荐执行顺序

1. 先改 `voice_speed=1.0` 默认不拼 `自然语速` 的逻辑。
2. 按个人分支流程提交、合并 `test`。
3. 部署/重启 h20 VoxCPM 容器。
4. 构造试听样本，验证短文案和长文案是否改善。
5. 同时构造完整视频任务，确认 `voice_emotion` 是否入库并进入 `mode=controllable`。
6. 如果长文案仍然偏平淡，再做 P1 分段生成方案。
